defmodule NewsbloatWeb.SidebarMenuComponent do
  ...
  def render(assigns) do
    ...
    <section
      ...
      class="lg:hidden fixed top-0 p-4 z-10"
    ...
    <section
      x-data="{ isOpen: true }"
      x-bind:class="isOpen ? 'w-64 p-4' : 'p-0 w-0'"
      class="flex-0 bg-background h-screen overflow-y-auto pb-16"
    >
    <button
      class="lg:m-0 bg-background group z-10" @click="isOpen = ! isOpen"
      x-bind:class="isOpen ? 'relative' : 'fixed top-4 left-4 '"
    >
      <%= icon_tag(@socket, "menu", class: "w-4 h-4 transform transition-transform group-hover:rotate-90") %>
    </button>
      <div class="mt-12">
